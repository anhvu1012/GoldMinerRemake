...
# claw variables
@export var claw_rotate_speed = 70
var claw_direction = 1
@export var claw_range = 80
...
func MoveRope(delta):
	...
	if !can_rotate: 
		...
		if hook_move_down:
			...
			if hook_position.y >= hook_max_depth or hook_position.x >= 1050 or hook_position.x <= 100:
				hook_direction *= -1
